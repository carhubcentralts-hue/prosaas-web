...
All voice MP3 files with Hebrew text: "היי, זאת הנציגה הדיגיטלית של פרו־סאס. תרצו אותי גם אצלכם בעסק?"
...
1. User selects a voice from the dropdown
2. User clicks "שמע דוגמה" (Hear Sample) button
3. App loads `/voices/{voice-name}.mp3?v=4` directly from this folder
4. If file exists → plays the audio with the fixed demo text
5. If file missing → shows "דמו לא זמין עדיין" (Demo not available yet)

**No API calls needed!** Everything works with static MP3 files.
**No free text input!** All demos play the same fixed Hebrew message.
...
- Shows warnings if files are missing or too small, but doesn't fail the build

This ensures only proper voice demos are used, while allowing the showcase site to build even with missing demos.
...
**Important:** After replacing files, update the cache version in `VoiceDemo.tsx` (currently `?v=4`) to ensure browsers fetch the new files.

## Note

The demo text is fixed and cannot be changed by users. This is intentional for the showcase site - it demonstrates different voice options with a consistent message.
